...
## Appendix: The Taste Maturity Model

SSOT is measured against a maturity ladder, from Level 0 (hardcoded chaos) to Level 5 (code-backed, where the design tool renders production components directly). Taste admits the same ladder, and reading the two side by side is instructive: an organization can be mature in *truth* (its values are normalized and synced) while primitive in *taste* (its standard still lives only in a few people's heads). The two maturities are independent, and the second is now the one that constrains outcomes.

| Level | State of Taste | Where the Standard Lives | Characteristics | Failure Mode |
| :--- | :--- | :--- | :--- | :--- |
| **0: Tacit** | None that is shared | Individual heads | Every maker relies on personal instinct. "Good" is whatever the last reviewer felt. Nothing is written. | Drift is total and invisible. The product's coherence depends entirely on who happened to touch it. |
| **1: Reference** | Aspirational, static | A brand deck, mood board, or PDF | Taste is *described* (adjectives, inspiration, do/don't images) but not *operable*. It informs; it cannot govern. | Document rot. The deck says "confident and calm"; the shipped screens say otherwise, and nothing reconciles them. |
| **2: Encoded but Flat** | Partial, context-free | A token and component library | Taste is partly captured as values and reusable parts, but as flat facts ("the blue is `#3b82f6`"), not as judgment. No policy, no context, no governance. Figma and code drift. | Consistency of *values* masquerades as taste. The system is uniform and still tasteless, because uniformity is not judgment. |
| **3: Contextual Policy** | Explicit, resolvable | A policy model over context coordinates | Taste is authored as resolution policies (the coordinate model): the right outcome per theme, density, agent, and state, with testable behavioral intent. But authoring is one-directional and lightly governed. | The standard is real but fragile. Without version control and review, a change to the policy is an unattributable, unrevertible event. |
| **4: Governed Source** | Explicit, versioned, owned | The design surface, backed by Git | The standard is branchable, reviewable, diffable, revertible, and bi-directionally synced (a Git-backed design tool such as GitFig). A taste change is a pull request. Design masters the standard with the rigor once exclusive to code. | Requires a Taste Steward and real governance discipline; the tooling enables coherence but does not supply the judgment. |
| **5: Generative Constraint** | Explicit and enforced at the point of making | The constraint set fed to every maker | The governed standard conditions all production: it is the guardrail handed to human builders and to generative systems, resolved automatically so that off-standard output is hard to produce in the first place. The design system is the operating system for taste. | Over-constraint. A standard strong enough to condition every generator can also flatten the human residue it exists to protect. The steward's job becomes knowing what *not* to encode. |

Two observations follow.

First, the jump that matters most today is **Level 3 to Level 4**: from taste that is merely *written down* to taste that is *governed*. This is exactly the jump the rest of this document argues has only just become possible, because Git-backed design tooling finally gives the design surface the version control, review, and blast-radius awareness that were the standing objection to letting design hold the canonical standard. Below Level 4, taste can be expressed but not *owned* as an evolving asset; at Level 4 it can.

Second, **Level 5 is where democratized building either pays off or destroys the product.** When every maker, human and machine, generates against the governed standard, a mature source of taste turns abundance into leverage: more can be built, and it is still good. An immature one turns the same abundance into entropy at machine speed. The difference between the two outcomes is entirely how far up this ladder the organization climbed before it handed the tools to everyone.
